...
class Robot (Base_Robot):
  ...
  def Output(self):
    dir = 0
    if self.ir_v in [1,2,3]: dir = -90
    elif self.ir_v in [4,5,6]: dir = 0
    elif self.ir_v in [7,8,9]: dir = 90
    else: dir = 180

    if dir == 180:
      self.motors.RadialMove(dir, speed = 100, relativeToField=True)
    else:
    ...
    if self.sideWall < 14:
      ...
      self.motors.RadialMove(wallDir, relativeToField=True)

    # Move forwards away from corners
    if (50 <= self.sideWall) and (self.sideWall <= 97):
      if self.backWall < 1:
        R.motors.RadialMove(0, relativeToField=True)
